...
class _DeactivateDemoScreenState extends State<DeactivateDemoScreen> {
 ...
 void reorderData(int oldindex, int newindex){
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("Deactivate Demo Screen"),),
      ...
      }, itemCount: list.length, onReorder: (int oldIndex,int newIndex){
...
class _DeactivateListItemState extends State<DeactivateListItem> {
 ...
 void deactivate(){
  ...
  Widget build(BuildContext context) {
    ...
    decoration: BoxDecoration(color: Colors.blue,border:Border.all(width: 5,color: Colors.black45)),
